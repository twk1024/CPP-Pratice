...
int main()
{
    ...
    /* 
      문제이름: 다이어트
      문제: https://koi.or.kr/assets/koi/2020/1/problems/m2-problems.pdf
    */
    ...
    int p[n], f[n], s[n], v[n], cst[n]; // 단백질, 지방, 탄수화물, 비타민, 가격
    for(int i=0;i<n;i++)
    {
            ...
            if(i!=j)
            {
                ...
                for(int q=0;q<n;q++)
                {
                    if(q!=j && q!=i && i!=j)
                    {
                        ...
                        for(int k=0;k<n;k++)
                        {
                            if(q!=j && q!=i && i!=j && k!=q && k!=j && k!=i)
                            {
                                cost=cst[i]+cst[j]+cst[q]+cst[k];
